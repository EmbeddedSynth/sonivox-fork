...
EAS_PUBLIC EAS_RESULT EAS_SetParameter (EAS_DATA_HANDLE pEASData, EAS_I32 module, EAS_I32 param, EAS_I32 value)
{
    ...
    {
        switch (param) {
        case EAS_PARAM_CHORUS_OVERRIDE_CC:
            if (value != 0 && pEASData->pVoiceMgr->chorusModule.effectData != NULL)
            {
                VMShutdownChorus(pEASData->pVoiceMgr);
                return EAS_SUCCESS;
            }
            if (value == 0 && pEASData->pVoiceMgr->chorusModule.effectData == NULL)
            {
        ...
        case EAS_PARAM_CHORUS_BYPASS:
            if (pEASData->pVoiceMgr->chorusModule.effectData != NULL)
            {
                EAS_Report(_EAS_SEVERITY_ERROR, "Setting EAS_PARAM_CHORUS_BYPASS while not overriding CC is not allowed\n");
                return EAS_ERROR_INVALID_PARAMETER;
            }
        default:
            break;
    ...
    {
        switch (param) {
        case EAS_PARAM_REVERB_OVERRIDE_CC:
            if (value != 0 && pEASData->pVoiceMgr->reverbModule.effectData != NULL)
            {
                VMShutdownReverb(pEASData->pVoiceMgr);
                return EAS_SUCCESS;
            }
            if (value == 0 && pEASData->pVoiceMgr->reverbModule.effectData == NULL)
            {
        ...
        case EAS_PARAM_REVERB_BYPASS:
            if (pEASData->pVoiceMgr->reverbModule.effectData != NULL)
            {
                EAS_Report(_EAS_SEVERITY_ERROR, "Setting EAS_PARAM_REVERB_BYPASS while not overriding CC is not allowed\n");
                return EAS_ERROR_INVALID_PARAMETER;
            }
        default:
            break;
